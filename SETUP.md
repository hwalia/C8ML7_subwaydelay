...
## Necessary Packages
The "C8ML7_SUBWAYDELAY" project uses its own isolated environment called `c8ml7-env` so that packages don’t conflict with other projects. 
...
1. Create a virtual environment called `c8ml7-env`:
    ```
    uv venv c8ml7-env --python 3.11
    ```

2. Activate the environment:
    - for macOS/Linux:
        ```
        source c8ml7-env/bin/activate
        ```
        
    - for windows (git bash):    
        ```
        source c8ml7-env/Scripts/activate
        ```
...
For questions or issues, please contact the "cohort-8 machine learning team 7" project learning support team or email courses.dsi@utoronto.ca.